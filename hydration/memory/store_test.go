...
func TestStoreApplySnapshotAndViewAreDefensive(t *testing.T) {
	store := New()
	sid := sessionstream.SessionId("s-1")
	payload, err := structpb.NewStruct(map[string]any{"text": "hello"})
	require.NoError(t, err)

	require.NoError(t, store.Apply(context.Background(), sid, 3, []sessionstream.TimelineEntity{{
		Kind:    "LabMessage",
		Id:      "msg-1",
		Payload: payload,
	}}))
...
func TestStoreSupportsTombstones(t *testing.T) {
	...
	require.NoError(t, store.Apply(context.Background(), sid, 1, []sessionstream.TimelineEntity{{
		Kind:    "LabMessage",
		Id:      "msg-1",
		Payload: payload,
	}}))
	require.NoError(t, store.Apply(context.Background(), sid, 2, []sessionstream.TimelineEntity{{
		Kind:      "LabMessage",
		Id:        "msg-1",
		Tombstone: true,
	}}))
