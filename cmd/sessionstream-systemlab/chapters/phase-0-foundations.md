...
When you finish Phase 0, you should understand what `sessionstream` is trying to become, why it lives in its own package, why Systemlab is a separate app, and what invariants the rest of the framework will build on. Phase 0 is not exciting in the way later phases are exciting. It is important in the way that foundations are important: everything else depends on it.

---

## 1. The story this framework is trying to tell

`sessionstream` is trying to become a reusable substrate for realtime, event-streaming applications. The phrase "event-streaming substrate" can sound abstract, so let me translate it.
...
## 2. Why we created `sessionstream` instead of renaming `pkg/webchat`
...
sessionstream → reusable substrate
```

That distinction will help you make better decisions. When you wonder where a helper, projection, or transport concept belongs, ask: is this substrate logic or application logic? Substrate logic belongs in `sessionstream`. Application logic belongs in a consumer package.
...
Systemlab lives at `sessionstream/cmd/sessionstream-systemlab`. It is all of these at once:
...
Systemlab may:
- import public `sessionstream` APIs,
...
│   └── transport.go
├── cmd/
│   └── sessionstream-systemlab/  ← owns the teaching app
│       ├── main.go
│       ├── server.go
│       ├── chapters/
│       └── static/
...
When you read that tree slowly, you can already see the intended ownership model. `sessionstream` owns the substrate. `cmd/sessionstream-systemlab` owns the teaching app. `Makefile` owns the repeatable validation entrypoints.
...
make test             # targeted framework tests
make systemlab-build  # clean build
make boundary-check   # catch violations
make check            # main validation path
```
...
- Donor code (`pkg/webchat`) and substrate (`sessionstream`) are different things with different purposes.
...
- `cmd/sessionstream-systemlab/main.go` — entrypoint
- `cmd/sessionstream-systemlab/server.go` — HTTP server and routing
- `cmd/sessionstream-systemlab/static/` — frontend structure
