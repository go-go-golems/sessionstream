...
func TypeScriptModule() *spec.Module {
	return &spec.Module{Name: ModuleName, RawDTS: []string{
		...
		"export interface Publisher { publish(name: string, payload: unknown): void; publishAsync(name: string, payload: unknown): Promise<void>; }",
		...
		"export interface Hub { submit(sessionId: string, name: string, payload: unknown): void; submitAsync(sessionId: string, name: string, payload: unknown): Promise<void>; snapshot(sessionId: string): Snapshot; command(name: string, handler: (cmd: Command, session: Session, publisher: Publisher) => void | Promise<void>): this; uiProjection(handler: (event: Event, session: Session, view: TimelineView) => UIEvent[] | Promise<UIEvent[]>): this; timelineProjection(handler: (event: Event, session: Session, view: TimelineView) => TimelineEntity[] | Promise<TimelineEntity[]>): this; run(): void; shutdown(): void; }",
