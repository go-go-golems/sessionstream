...
func TypeScriptModule() *spec.Module {
	return &spec.Module{Name: ModuleName, RawDTS: []string{
		...
		"export interface Publisher { publish(name: string, payload: unknown): void; }",
		...
		"export interface Hub { submit(sessionId: string, name: string, payload: unknown): void; snapshot(sessionId: string): Snapshot; command(name: string, handler: (cmd: Command, session: Session, publisher: Publisher) => void): this; uiProjection(handler: (event: Event, session: Session, view: TimelineView) => UIEvent[]): this; timelineProjection(handler: (event: Event, session: Session, view: TimelineView) => TimelineEntity[]): this; run(): void; shutdown(): void; }",
